				...
				<form action="joinForm" id="joinForm" method="post">
					...
					<div class="form-group email-form">
						<label for="email">이메일</label><br> <input type="text"
							class="form-control" name="userEmail1" id="userEmail1"
							placeholder="이메일"> <select class="form-control" name="userEmail2"
							id="userEmail2">
					...
					<div class="form-group">
						<button type="button" class="btn btn-lg btn-info btn-block" onclick="location.gref='userLogin'">로그인</button>
					</div>
				</form>
			</div>
		</div>
...
<script>
	...
	$("#joinBtn").click(function(){
		if($("#userId").attr("readonly") !='readonly') {//중복검사유무 확인
	...
	//편의성처리 - 엔터값처리(form태그에 keyup이벤트, 엔터값이 아니라면 처리x)
...
<script>
	...
	function jusoCallBack(roadFullAddr,roadAddrPart1,addrDetail,roadAddrPart2,engAddr, jibunAddr, zipNo, admCd, rnMgtSn, bdMgtSn,detBdNmList,bdNm,bdKdcd,siNm,sggNm,emdNm,liNm,rn,udrtYn,buldMnnm,buldSlno,mtYn,lnbrMnnm,lnbrSlno,emdNo){
		...
		document.getElenebtById("addrZipnum").value = zipNo;
